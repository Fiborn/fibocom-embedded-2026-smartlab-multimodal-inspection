...
private:
  ...
  // 手势停车服务
  rclcpp::Service<std_srvs::srv::Trigger>::SharedPtr stop_inspection_service_;

  // 紧急停车发布器 (手势停车兜底)
  rclcpp::Publisher<geometry_msgs::msg::Twist>::SharedPtr cmd_vel_pub_;

  // 启动后延迟连接华为云（一次性定时器）
  rclcpp::TimerBase::SharedPtr connect_cloud_timer_;
  void connect_cloud_on_startup();
  ...
  std::atomic<bool> stop_requested_;   // 手势停车标志
  ...
  void stop_inspection_callback(const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
                                std::shared_ptr<std_srvs::srv::Trigger::Response> response);
