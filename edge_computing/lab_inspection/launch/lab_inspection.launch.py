...
def generate_launch_description():
    ...
    # 8. 手势识别桥接节点 (Awesome(666)发车 / Victory(Yeah)停车)
    #    依赖 X3 平台的 /tros_perc_fusion 话题
    #    手势识别的实时可视化效果通过 X3 自带的 Web 显示(图像编码+浏览器展示)
    gesture_bridge_node = Node(
        ...
        output='screen',
        parameters=[{
            'ai_msg_sub_topic': '/hobot_hand_static_gesture_detection',
            'debounce_interval': 8.0,
            'hold_time': 1.2,
            'grace_period': 0.5,
            'start_inspection_service': '/start_inspection',
            'stop_inspection_service': '/stop_inspection',
            'cmd_vel_topic': '/cmd_vel',
            'verbose_log': True,
        }]
    )

    # 9. Web 桥接节点 — HTTP→ROS2 服务代理（网页端发车/停车）
    web_bridge_node = Node(
        package='lab_inspection',
        executable='web_bridge.py',
        name='web_bridge',
        output='screen',
        parameters=[{
            'bridge_port': 5000,
