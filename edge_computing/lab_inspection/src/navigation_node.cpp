...
static const std::chrono::seconds kNavActionServerTimeout(5);
    ...
    stop_requested_(false)
{
  ...
  // 创建手势停车服务
  stop_inspection_service_ = this->create_service<std_srvs::srv::Trigger>(
      "/stop_inspection",
      std::bind(&NavigationNode::stop_inspection_callback, this,
                std::placeholders::_1, std::placeholders::_2));
  ...
  // 启动后延迟连接华为云（等待 vision_node 的 /connect_huawei_cloud 服务就绪）
  connect_cloud_timer_ = this->create_wall_timer(
      std::chrono::seconds(8),
      std::bind(&NavigationNode::connect_cloud_on_startup, this));
}
...
void NavigationNode::load_workstations()
{
  ...
  workstations_.emplace_back(1, "工位0", 2.3, -0.9,-3.15);    
  workstations_.emplace_back(2, "工位1", 2.3, -1.50, -3.15);   
  workstations_.emplace_back(3, "工位2", 2.3, -2.1, -3.15);   
...
bool NavigationNode::navigate_to_pose(double x, double y, double yaw)
{
  ...
  RCLCPP_INFO(get_logger(), "[计时] wait_for_action_server 耗时: %.0fms",
              std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count());

  // 重试机制：当目标被拒绝时（生命周期节点尚未激活），等待后重试
  // 若目标被接受但导航执行失败，则不重试直接返回
  const int kMaxRetries = 5;
  const auto kRetryDelay = std::chrono::milliseconds(200);
  ...
  for (int retry = 0; retry < kMaxRetries; ++retry) {
    ...
    auto nav_ms = std::chrono::duration_cast<std::chrono::milliseconds>(t_done - t_send).count();
    RCLCPP_INFO(get_logger(), "[计时] 导航执行耗时: %.1fs (含路径规划+移动)", nav_ms / 1000.0);
...
void NavigationNode::stop_inspection_callback(
    const std::shared_ptr<std_srvs::srv::Trigger::Request>,
    std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
  if (!inspection_running_.load()) {
    RCLCPP_INFO(get_logger(), "当前无巡检任务运行，无需停车");
    response->success = true;
    response->message = "当前无巡检任务";
    return;
  }

  RCLCPP_INFO(get_logger(), "收到手势停车请求，正在停止巡检...");
  stop_requested_.store(true);

  // 取消当前导航目标 (如果在导航中)
  if (nav_action_client_) {
    RCLCPP_INFO(get_logger(), "取消所有导航目标...");
    nav_action_client_->async_cancel_all_goals();
  }

  // 模拟按键跳过，让 wait_for_vision_or_key 也能退出
  key_pressed_.store(true);
  ...
  response->success = true;
  response->message = "巡检已停止";
  RCLCPP_INFO(get_logger(), "停车指令已执行");
}
...
void NavigationNode::connect_cloud_on_startup()
{
  // 取消定时器（一次性执行）
  connect_cloud_timer_->cancel();

  RCLCPP_INFO(get_logger(), "==================================================");
  RCLCPP_INFO(get_logger(), "自动连接华为云 IoT 平台（启动后自动执行）");
  RCLCPP_INFO(get_logger(), "==================================================");

  if (connect_cloud_client_->wait_for_service(std::chrono::seconds(15))) {
    auto req = std::make_shared<std_srvs::srv::Trigger::Request>();
    auto future = connect_cloud_client_->async_send_request(req);
    if (future.wait_for(std::chrono::seconds(50)) == std::future_status::ready) {
      auto res = future.get();
      if (res->success) {
        RCLCPP_INFO(get_logger(), "华为云连接成功: %s", res->message.c_str());
      } else {
        RCLCPP_ERROR(get_logger(), "华为云连接失败: %s，巡逻时数据将无法上报", res->message.c_str());
      }
    } else {
      RCLCPP_ERROR(get_logger(), "华为云连接超时，巡逻时数据将无法上报");
    }
  } else {
    RCLCPP_ERROR(get_logger(), "华为云连接服务不可用，巡逻时数据将无法上报");
  }
}
...
void NavigationNode::run_inspection()
{
  ...
  RCLCPP_INFO(get_logger(), "==================================================");
  RCLCPP_INFO(get_logger(), "巡检开始（跳过初始位姿设置，AMCL 已自动维护定位）");
  RCLCPP_INFO(get_logger(), "==================================================");

  // 依次导航到各个工位
  RCLCPP_INFO(get_logger(), "开始工位巡航...");

  for (size_t i = 0; i < workstations_.size(); ++i) {
    // 检查是否收到停车请求
    if (stop_requested_.load()) {
      RCLCPP_INFO(get_logger(), "收到停车信号，中止巡检");
      break;
    }
    ...
    // 导航到当前工位
    auto t_nav_start = std::chrono::steady_clock::now();
    RCLCPP_INFO(get_logger(), "[计时] 开始导航到工位%d (%s)", ws.id, ws.name.c_str());
    bool nav_success = navigate_to_pose(ws.x, ws.y, ws.yaw);
    auto t_nav_elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::steady_clock::now() - t_nav_start).count();
    RCLCPP_INFO(get_logger(), "[计时] 导航到工位%d 耗时: %.1fs", ws.id, t_nav_elapsed / 1000.0);
  ...
  RCLCPP_INFO(get_logger(), "[计时] 巡检总耗时: %.1fs", t_total / 1000.0);
}
